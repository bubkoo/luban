...
if (typeof window === 'undefined') {
  let fetchMod = eval('require')('node-fetch'); // tslint:disable-line
  FETCH = global.fetch || fetchMod
  REQUEST = global.Request || fetchMod.Request
  HEADERS = global.Headers || fetchMod.Headers
  WEBSOCKET = global.WebSocket || eval('require')('ws') // tslint:disable-line
} else {
  FETCH = fetch
  REQUEST = Request
  HEADERS = Headers
...
namespace Private {
  export function makeSettings(
    ...
    const baseUrl = URLExt.normalize(options.baseUrl) || defaultSettings.baseUrl
